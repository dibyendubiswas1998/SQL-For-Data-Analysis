...
select c.customer_code, c.customer, avg(inv.pre_invoice_discount_pct) as average_discount_percentage from dim_customer as c
...
from fact_sales_monthly group by Quarter order by Total_sold_quantity desc limit 1;
...
select s1.channel,  round((s1.sold_quantity * s2.gross_price), 0) as gross_sales_mln
from ss1 as s1
...
rank () over (order by p.product) as rank_order
from dim_product as p
join fact_sales_monthly as s
on p.product_code = s.product_code
where s.fiscal_year = 2021
group by p.product order by total_sold_quantity desc limit 3;
